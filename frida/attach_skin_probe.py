"""Attach M8P3.8 skin update probe. See JS for protocol.

Usage (admin shell):
    python frida\attach_skin_probe.py > frida\skin_probe.log
"""
import sys, time, io
from pathlib import Path
import frida

# Force UTF-8 on stdout/stderr to avoid cp1252 encode errors on Windows
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8', errors='replace')

SCRIPT = Path(__file__).parent / "18_skin_update_hook_probe.js"
